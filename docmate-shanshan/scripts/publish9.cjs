...
const dest = path.join(root, 'publish9')
const PUBLISH_NAME = 'publish9'
...
function clearDestExceptLocked() {
  ...
  try {
    ...
    console.warn('publish9 目录被占用，改为增量同步（请先关闭正在运行的 DocMate）')
  }
}
...
fs.writeFileSync(
  ...
  `# DocMate 闪闪文档 v${pkg.version}（完整发布包 publish9）
