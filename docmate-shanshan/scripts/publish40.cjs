...
const dest = path.join(root, 'publish40')
const PUBLISH_NAME = 'publish40'
...
function clearDestExceptLocked() {
  ...
  try {
    ...
    console.warn('publish40 目录被占用，改为增量同步（请先关闭 DocMate）')
  }
}
...
try {
  console.log('正在安装 publish40 运行依赖（首次约 1 分钟）…')
  ...
  console.warn('自动安装依赖失败，请手动在 publish40 目录运行: npm install --omit=dev')
  console.warn(err.message || err)
}

fs.writeFileSync(
  path.join(dest, '使用说明.txt'),
  `DocMate v${pkg.version} — publish40

【推荐启动】双击 DocMate.vbs

【新特性】
· Agent 对话红删绿增 Diff，点击同步到文档
· 精简 Agent 面板布局
· Ctrl+Enter 采纳 · Esc 拒绝
`,
  'utf-8',
)
