...
const PUBLISH_NAME = process.env.PUBLISH_NAME || 'publish70'
const dest = path.join(root, PUBLISH_NAME)
...
function clearDestExceptLocked() {
  ...
  try {
    ...
    console.warn(`${PUBLISH_NAME} 目录被占用，改为增量同步（请先关闭 DocMate）`)
  }
}
...
function cleanTarget(relativePath) {
  ...
  try {
    ...
    if (err.code !== 'EPERM' && err.code !== 'EBUSY') throw err
    console.warn(`${relativePath} 被占用，保留旧文件并覆盖可写文件`)
  }
}
...
for (const dir of INCLUDE_DIRS) {
  ...
  if (fs.existsSync(srcDir)) {
    if (incrementalSync) cleanTarget(dir)
...
if (fs.existsSync(distSrc)) {
  if (incrementalSync) cleanTarget('dist')
...
try {
  console.log(`正在安装 ${PUBLISH_NAME} 运行依赖（首次约 1 分钟）…`)
  ...
  console.warn(`自动安装依赖失败，请手动在 ${PUBLISH_NAME} 目录运行: npm install --omit=dev`)
  console.warn(err.message || err)
}

fs.writeFileSync(
  path.join(dest, '使用说明.txt'),
  `DocMate v${pkg.version} — ${PUBLISH_NAME}

【推荐启动】双击 DocMate.vbs 或 启动DocMate.bat

【本版优化】
· Bubble 面板任务统一同步到右侧闪闪聊天区
· 当前选中文字随任务一起进入闪闪上下文
· 新增在线优先语音识别，可配置 OpenAI 兼容 audio/transcriptions
· 在线识别失败自动回退本地 Whisper tiny
· 保留三栏布局、面板模式切换和 UI 动画增强
`,
  'utf-8',
)
