...
const dest = path.join(root, 'publish60')
const PUBLISH_NAME = 'publish60'
...
function clearDestExceptLocked() {
  ...
  try {
    ...
    console.warn('publish60 目录被占用，改为增量同步（请先关闭 DocMate）')
  }
}
...
try {
  console.log('正在安装 publish60 运行依赖（首次约 1 分钟）…')
  ...
  console.warn('自动安装依赖失败，请手动在 publish60 目录运行: npm install --omit=dev')
  console.warn(err.message || err)
}

fs.writeFileSync(
  path.join(dest, '使用说明.txt'),
  `DocMate v${pkg.version} — publish60

【推荐启动】双击 DocMate.vbs 或 启动DocMate.bat

【本版优化】
· 三栏布局：文件 | 编辑器 | Agent 并排，默认侧栏模式
· 面板可切换侧栏/浮窗，布局偏好 localStorage 记忆
· TitleBar 精简为 [文件] [Agent]，设置收入 ··· 菜单
· Diff 暗色 rgba、弹窗/状态/Toast 微动画
· BubbleMenu 胶囊快捷 + / 斜杠命令
· Ctrl+K 切换 Agent 面板
`,
  'utf-8',
)
