...
const { transcribeAudioBuffer } = require('./speech.cjs')
...
const APP_ICON = path.join(__dirname, '../public/闪闪文档.png')
let mainWindow = null
...
function buildAiContext(userDataPath) {
  ...
  const allChunks = readAllChunks(userDataPath)
  const preferences = readUserProfile(userDataPath)
  const legacyKnowledgeBase = allChunks.length ? '' : readKnowledgeBase(userDataPath)
...
let currentAiAbortController = null
  ...
  ipcMain.handle('ai:process', async (event, task, payload) => {
    ...
    const aiContext = buildAiContext(userDataPath)
    if (currentAiAbortController) {
  ...
  // 知识库管理（旧接口保留兼容）
  ipcMain.handle('knowledge:get', () => readKnowledgeBase(app.getPath('userData')))
  ipcMain.handle('knowledge:save', (_e, content) => {
    writeKnowledgeBase(app.getPath('userData'), content)
    invalidateAiContext()
    return { ok: true }
  })

  ipcMain.handle('kb:list-docs', () => listDocuments(app.getPath('userData')))
  ipcMain.handle('kb:get-chunks', () => readAllChunks(app.getPath('userData')))
  ...
  ipcMain.handle('kb:import-doc', async (event) => {
    ...
    const saved = persistImportedDoc(app.getPath('userData'), name, chunks)
    invalidateAiContext()
  ...
  ipcMain.handle('kb:import-text', async (event, { name, content }) => {
    ...
    const saved = persistImportedDoc(app.getPath('userData'), docName, chunks)
    invalidateAiContext()
  ...
  ipcMain.handle('kb:delete-doc', (_e, docId) => {
    const userDataPath = app.getPath('userData')
    removeDocumentMeta(userDataPath, docId)
    removeChunksByDocId(userDataPath, docId)
    invalidateAiContext()
    return { ok: true }
  })

  ipcMain.handle('prefs:get', () => readUserProfile(app.getPath('userData')))
  ipcMain.handle('prefs:update', (_e, updates) => {
    ...
    writeUserProfile(userDataPath, merged)
    invalidateAiContext()
    return merged
  })
  ipcMain.handle('prefs:reset', () => {
    ...
    writeUserProfile(userDataPath, profile)
    invalidateAiContext()
    try {
      fs.writeFileSync(path.join(userDataPath, 'interaction-log.jsonl'), '', 'utf-8')
    } catch {
      /* ignore */
    }
    return profile
  ...
  ipcMain.handle('log-interaction', (_e, record) => {
    const userDataPath = app.getPath('userData')
    const settings = readSettings()
    const result = logInteraction(userDataPath, record, settings, callChat)
    invalidateAiContext()
    return result
  })
...
app.whenReady().then(async () => {
  ensureWorkspace()
  registerIpc()
  migrateLegacyKnowledgeBase(app.getPath('userData')).catch((err) => {
