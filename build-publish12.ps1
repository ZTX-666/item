# publish12: HiAgent bridge + idempotent env setup (赤瞳灵讯)
# Usage: powershell -ExecutionPolicy Bypass -File build-publish12.ps1

$ErrorActionPreference = "Stop"
$Root = Split-Path -Parent $MyInvocation.MyCommand.Path
$Project = Join-Path $Root "WacliDesktop\WacliDesktop.csproj"
$Publish12 = Join-Path $Root "publish12"
$SourceOut = Join-Path $Publish12 "source"
$PortableOut = Join-Path $Publish12 "portable"
$RuntimeOut = Join-Path $Publish12 "runtime"
$SingleExe = Join-Path $Publish12 "ChitongLingxun-standalone.exe"
...
Write-Host "[1/9] Clean publish12 ..."
if (Test-Path $Publish12) { Remove-Item $Publish12 -Recurse -Force }
...
赤瞳灵讯 · runtime 目录
========================
「配置环境」：扫描本机工具 → 复用或 winget 安装 → 保存 environment.json
「HiAgent对接」：一键启动本机 API + 内网穿透，复制公网地址与 API Key

整包拷贝 publish12 可迁移；新电脑需重新「配置环境」与「HiAgent对接」。
"@
...
Stop-WacliDesktopApps
$tempSingle = Join-Path $env:TEMP ("wacli-publish12-single-" + [guid]::NewGuid().ToString("n"))
...
$exclude = @('bin', 'obj', 'publish', 'publish1', 'publish2', 'publish3', 'publish4', 'publish5', 'publish6', 'publish7', 'publish8', 'publish9', 'publish10', 'publish11', 'publish12', '.vs')
...
Set-Content -Path (Join-Path $Publish12 "start.bat") -Encoding ASCII -Value '@start "" "%~dp0portable\WacliDesktop.exe"'
...
Copy-Item (Join-Path $env:TEMP "wacli-launcher-out\wacli-desktop.exe") (Join-Path $Publish12 "wacli-desktop.exe") -Force

Write-Host "[9/9] Done"
Write-Host ""
Write-Host "Done - publish12 (赤瞳灵讯 · HiAgent对接)"
