...
def _replace_in_paragraph(para, target: str, replacement: str) -> bool:
    """Replace the first occurrence of ``target`` inside a paragraph.

    python-docx splits a paragraph into multiple runs (e.g. digits or differently
    formatted spans become their own run), so the naive ``target in run.text``
    check fails whenever the target spans more than one run — which silently
    dropped most edits and produced an unchanged output file. This handles both
    cases: a fast in-run path that preserves formatting, and a cross-run path
    that rebuilds the runs when the target straddles a run boundary.
    """
    if not target or target not in para.text:
        return False

    # Fast path: target lives entirely within a single run — keep its formatting.
    for run in para.runs:
        if target in run.text:
            run.text = run.text.replace(target, replacement, 1)
            return True

    # Cross-run path: collapse the target span into the first overlapping run and
    # clear the remainder. Formatting of the first run wins for the new text.
    runs = para.runs
    if not runs:
        return False
    ...
    for run in runs:
        ...
        if r_end <= idx or r_start >= end:
            # Run is entirely outside the target span — leave untouched.
            continue
        # Run overlaps the target span.
        prefix = run.text[: max(0, idx - r_start)] if r_start < idx else ""
        suffix = run.text[end - r_start :] if r_end > end else ""
        if not wrote:
    ...
    for change in accepted_changes:
        ...
        try:
            if ctype == "text_append":
                ...
                applied = False
                # Search and replace in paragraphs (handles multi-run targets).
...
# ---------------------------------------------------------------------------
# Document retrieval + LLM-produced changeset registration
#
# These two tools let the orchestration layer (chitung-center) drive the
# "intelligent" part through its unified LLM gateway:
#   1. docmate_get_document      → fetch stored paragraphs to build the prompt
#   2. docmate_register_changeset → turn LLM-produced edits into a real
#                                   changeset (reusing the same preview-card and
#                                   apply contract as the pattern matcher)
# ---------------------------------------------------------------------------

def docmate_get_document(doc_id: str) -> ToolResult:
    """Return the stored paragraphs/stats for a loaded document by doc_id."""
    task_id = new_task_id("docmate")
...
def docmate_register_changeset(doc_id: str, instruction: str, changes: list[dict[str, Any]]) -> ToolResult:
    """Build and store a changeset from explicit (LLM-produced) edits.

    Each edit: {"type": "replace|delete|append", "target": str, "replacement": str}.
    Reuses _make_changeset so preview cards and the apply contract stay identical
    to the pattern-matching path.
    """
    task_id = new_task_id("docmate")
    doc = _doc_store.get(doc_id)
    if not doc:
    ...
    for edit in changes or []:
        if not isinstance(edit, dict):
            continue
        ptype = _normalize_edit_type(edit.get("type"))
...
ALL_DOCMATE_SPECS = [
    ...
    ToolSpec(
        ...
        description="将 LLM 生成的编辑项注册为可预览、可应用的 changeset（与模式匹配路径共用同一应用契约）。",
        input_schema={
            "type": "object",
            "properties": {
                "doc_id": {"type": "string"},
