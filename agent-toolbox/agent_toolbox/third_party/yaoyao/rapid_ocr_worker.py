    ...
    for line in sys.stdin:
        ...
        try:
            ...
            if not image_path.exists() or image_path.stat().st_size <= 0:
                print(json.dumps({"id": req_id, "ok": False, "error": "input image is empty or missing."}), flush=True)
                continue
...
def main() -> int:
    ...
    if not image_path.exists() or image_path.stat().st_size <= 0:
        out_path.write_text("ERROR:input image is empty or missing.", encoding="utf-8")
        return 2
