# publish7: source + portable folder + single-file exe (赤瞳灵讯)
# Usage: powershell -ExecutionPolicy Bypass -File build-publish7.ps1

$ErrorActionPreference = "Stop"
$Root = Split-Path -Parent $MyInvocation.MyCommand.Path
$Project = Join-Path $Root "WacliDesktop\WacliDesktop.csproj"
$Publish7 = Join-Path $Root "publish7"
$SourceOut = Join-Path $Publish7 "source"
$PortableOut = Join-Path $Publish7 "portable"
$SingleExe = Join-Path $Publish7 "ChitongLingxun-standalone.exe"
...
Write-Host "[1/8] Clean publish7 ..."
if (Test-Path $Publish7) { Remove-Item $Publish7 -Recurse -Force }
...
Stop-WacliDesktopApps
$tempSingle = Join-Path $env:TEMP ("wacli-publish7-single-" + [guid]::NewGuid().ToString("n"))
...
$exclude = @('bin', 'obj', 'publish', 'publish1', 'publish2', 'publish3', 'publish4', 'publish5', 'publish6', 'publish7', '.vs')
...
Set-Content -Path (Join-Path $Publish7 "start.bat") -Encoding ASCII -Value '@start "" "%~dp0portable\WacliDesktop.exe"'
