...
function attachChangeSummary(result) {
  const oldText = result.operation === 'insert' ? '' : String(result.old_text || '')
  const nextText = result.operation === 'delete' ? '' : String(result.options?.[0]?.text || '')
  const added = countVisibleChars(nextText)
  const removed = countVisibleChars(oldText)
  const summary = `+${added} -${removed}`
  result.change_summary = summary
  result.reason = [summary, result.reason].filter(Boolean).join('  ')
  return result
}
...
只输出 JSON。${optionsHint}${opHint}`)
  ...
  try {
    return attachChangeSummary(validateRevisionResult(result, command))
  } catch (err) {
...
${optionsHint}`)
  ...
  return attachChangeSummary(validateRevisionResult(normalizeRevisionResult(parseJson(content, '润色结果 JSON'), document_text, selected_text || target, optionCount), tone))
}
...
options 数组只能有 1 项。`)
  ...
  return attachChangeSummary(validateRevisionResult(normalizeRevisionResult(parseJson(content, '口语转换 JSON'), oral_text, oral_text), style))
}
