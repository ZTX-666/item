...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  saveRuntimeConfig: (patch) => ipcRenderer.invoke('config:saveRuntime', patch),
  chooseRuntimeDirectory: (key) => ipcRenderer.invoke('config:chooseDirectory', key),
  openRuntimeDirectory: (key) => ipcRenderer.invoke('config:openDirectory', key),
  checkEnvironment: () => ipcRenderer.invoke('env:check'),
  setupSpeechEnvironment: () => ipcRenderer.invoke('env:setupSpeech'),
  onEnvironmentProgress: (cb) => {
    const handler = (_e, progress) => cb(progress)
    ipcRenderer.on('env:setup-progress', handler)
    return () => ipcRenderer.removeListener('env:setup-progress', handler)
  },
