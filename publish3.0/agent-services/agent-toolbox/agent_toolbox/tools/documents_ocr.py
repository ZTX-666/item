...
def ocr_document_or_image(req: OcrDocumentRequest) -> ToolResult:
    exists = Path(req.path).exists()
    return ToolResult(
        ok=True,
        tool="ocr_document_or_image",
        summary="OCR placeholder. Wire to PaddleOCRSharp/RapidOCR in the next pass.",
        data={"path": req.path, "exists": exists, "engine": req.engine, "text": ""},
    )
...
def extract_tables_from_document(req: TableExtractionRequest) -> ToolResult:
    ...
    return ToolResult(
        ...
        summary="Table extraction placeholder. Wire to python-docx/pdfplumber/Docling later.",
        data={"path": req.path, "exists": exists, "engine": req.engine, "tables": []},
    )
