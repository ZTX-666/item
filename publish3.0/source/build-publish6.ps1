# publish6: source + portable folder + single-file exe (赤瞳灵讯)
# Usage: powershell -ExecutionPolicy Bypass -File build-publish6.ps1

$ErrorActionPreference = "Stop"
$Root = Split-Path -Parent $MyInvocation.MyCommand.Path
$Project = Join-Path $Root "WacliDesktop\WacliDesktop.csproj"
$Publish6 = Join-Path $Root "publish6"
$SourceOut = Join-Path $Publish6 "source"
$PortableOut = Join-Path $Publish6 "portable"
$SingleExe = Join-Path $Publish6 "ChitongLingxun-standalone.exe"
...
Write-Host "[1/8] Clean publish6 ..."
if (Test-Path $Publish6) { Remove-Item $Publish6 -Recurse -Force }
...
Stop-WacliDesktopApps
$tempSingle = Join-Path $env:TEMP ("wacli-publish6-single-" + [guid]::NewGuid().ToString("n"))
...
$exclude = @('bin', 'obj', 'publish', 'publish1', 'publish2', 'publish3', 'publish4', 'publish5', 'publish6', '.vs')
...
Set-Content -Path (Join-Path $Publish6 "start.bat") -Encoding ASCII -Value '@start "" "%~dp0portable\WacliDesktop.exe"'
