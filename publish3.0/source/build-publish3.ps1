# publish3: source + portable folder + single-file exe
# Usage: powershell -ExecutionPolicy Bypass -File build-publish3.ps1

$ErrorActionPreference = "Stop"
$Root = Split-Path -Parent $MyInvocation.MyCommand.Path
$Project = Join-Path $Root "WacliDesktop\WacliDesktop.csproj"
$Publish3 = Join-Path $Root "publish3"
$SourceOut = Join-Path $Publish3 "source"
$PortableOut = Join-Path $Publish3 "portable"
$SingleExe = Join-Path $Publish3 "WhatsApp本地同步中台.exe"

Write-Host "[1/8] Clean publish3 ..."
if (Test-Path $Publish3) { Remove-Item $Publish3 -Recurse -Force }
...
$tempSingle = Join-Path $env:TEMP "wacli-publish3-single"
...
Move-Item (Join-Path $tempSingle "WacliDesktop.exe") $SingleExe -Force
...
foreach ($exe in @((Join-Path $PortableOut "WacliDesktop.exe"), $SingleExe)) {
    ...
    Stop-Process -Id $px.Id -Force
}
Write-Host "      PASS"

Write-Host "[7/8] Pack source ..."
$exclude = @('bin', 'obj', 'publish', 'publish1', 'publish2', 'publish3', '.vs')
