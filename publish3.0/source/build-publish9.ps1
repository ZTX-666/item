# publish9: source + portable folder + single-file exe (赤瞳灵讯)
# Usage: powershell -ExecutionPolicy Bypass -File build-publish9.ps1

$ErrorActionPreference = "Stop"
$Root = Split-Path -Parent $MyInvocation.MyCommand.Path
$Project = Join-Path $Root "WacliDesktop\WacliDesktop.csproj"
$Publish9 = Join-Path $Root "publish9"
$SourceOut = Join-Path $Publish9 "source"
$PortableOut = Join-Path $Publish9 "portable"
$SingleExe = Join-Path $Publish9 "ChitongLingxun-standalone.exe"
...
Write-Host "[1/8] Clean publish9 ..."
if (Test-Path $Publish9) { Remove-Item $Publish9 -Recurse -Force }
...
Stop-WacliDesktopApps
$tempSingle = Join-Path $env:TEMP ("wacli-publish9-single-" + [guid]::NewGuid().ToString("n"))
...
$exclude = @('bin', 'obj', 'publish', 'publish1', 'publish2', 'publish3', 'publish4', 'publish5', 'publish6', 'publish7', 'publish8', 'publish9', '.vs')
...
Set-Content -Path (Join-Path $Publish9 "start.bat") -Encoding ASCII -Value '@start "" "%~dp0portable\WacliDesktop.exe"'
