# publish11: idempotent one-click env (scan → reuse → install → manifest)
# Usage: powershell -ExecutionPolicy Bypass -File build-publish11.ps1

$ErrorActionPreference = "Stop"
$Root = Split-Path -Parent $MyInvocation.MyCommand.Path
$Project = Join-Path $Root "WacliDesktop\WacliDesktop.csproj"
$Publish11 = Join-Path $Root "publish11"
$SourceOut = Join-Path $Publish11 "source"
$PortableOut = Join-Path $Publish11 "portable"
$RuntimeOut = Join-Path $Publish11 "runtime"
$SingleExe = Join-Path $Publish11 "ChitongLingxun-standalone.exe"
...
Write-Host "[1/9] Clean publish11 ..."
if (Test-Path $Publish11) { Remove-Item $Publish11 -Recurse -Force }
...
赤瞳灵讯 · runtime 目录
========================
首次使用请点击软件内「配置环境」（一键完成）：
  · 扫描本机已安装的 Git / Go / gcc / wacli，能复用则直接复用
  · 缺失组件自动 winget 安装；wacli 可 git 编译到 runtime\bin
  · 路径保存在 environment.json

整包拷贝 publish11 即可迁移；新电脑再点一次「配置环境」。
"@
...
Stop-WacliDesktopApps
$tempSingle = Join-Path $env:TEMP ("wacli-publish11-single-" + [guid]::NewGuid().ToString("n"))
...
$exclude = @('bin', 'obj', 'publish', 'publish1', 'publish2', 'publish3', 'publish4', 'publish5', 'publish6', 'publish7', 'publish8', 'publish9', 'publish10', 'publish11', '.vs')
...
Set-Content -Path (Join-Path $Publish11 "start.bat") -Encoding ASCII -Value '@start "" "%~dp0portable\WacliDesktop.exe"'
...
Copy-Item (Join-Path $env:TEMP "wacli-launcher-out\wacli-desktop.exe") (Join-Path $Publish11 "wacli-desktop.exe") -Force

Write-Host "[9/9] Done"
Write-Host ""
Write-Host "Done - publish11 (赤瞳灵讯 · 一键配置环境)"
