# publish5: source + portable folder + single-file exe (赤瞳灵讯)
# Usage: powershell -ExecutionPolicy Bypass -File build-publish5.ps1

$ErrorActionPreference = "Stop"
$Root = Split-Path -Parent $MyInvocation.MyCommand.Path
$Project = Join-Path $Root "WacliDesktop\WacliDesktop.csproj"
$Publish5 = Join-Path $Root "publish5"
$SourceOut = Join-Path $Publish5 "source"
$PortableOut = Join-Path $Publish5 "portable"
$SingleExe = Join-Path $Publish5 "ChitongLingxun-standalone.exe"
...
Write-Host "[1/8] Clean publish5 ..."
if (Test-Path $Publish5) { Remove-Item $Publish5 -Recurse -Force }
...
Stop-WacliDesktopApps
$tempSingle = Join-Path $env:TEMP ("wacli-publish5-single-" + [guid]::NewGuid().ToString("n"))
...
$exclude = @('bin', 'obj', 'publish', 'publish1', 'publish2', 'publish3', 'publish4', 'publish5', '.vs')
...
Set-Content -Path (Join-Path $Publish5 "start.bat") -Encoding ASCII -Value '@start "" "%~dp0portable\WacliDesktop.exe"'
